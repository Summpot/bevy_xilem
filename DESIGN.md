...
- **Theme-file-first examples:** every example loads its own style asset (for example `assets/themes/calculator.ron`, `assets/themes/game_2048.ron`, `assets/themes/ui_showcase.ron`, `assets/themes/pixiv_client.ron`) via `AppBevyXilemExt::load_style_sheet(...)`; these files carry example-specific layout and visual overrides while built-in Fluent variants remain the baseline theme source.
