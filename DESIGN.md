...
- **Pixiv example UX:** `example_pixiv_client` uses a showcase-like sidebar navigation state, centered dialog-style illustration detail panel, masonry-style multi-column feed layout for mixed-height cards, immediate locale switch handling from combo events, and thumbnail-click detail open interaction (without dedicated card “Open” button / hover magnification). Sidebar tabs include Home/Rankings/Manga/Novels/Search; novel feed items are normalized into the card pipeline so mixed content can be projected consistently. The detail overlay is modal and uses dedicated per-card action entities for thumbnail-open/bookmark interactions to avoid click-routing conflicts. Auth credentials are persisted locally after successful login/refresh and restored on next launch for automatic refresh-based re-auth.
